...
db.dropCollection("recipes", function (err, result) {
  console.log("users dropped");
});
const main = async () => {
  const users = [
    {
      username: "mental",
      email: "mental@mental.com",
      password: await bcrypt.hash("treesap20", 11),
      admin: true,
    },
    {
      username: "marca",
      email: "marca@marca.com",
      password: await bcrypt.hash("bloonga20", 11),
      admin: true,
    },
  ];
  const savedUsers = await User.insertMany(users);
  console.log(savedUsers[1]);
